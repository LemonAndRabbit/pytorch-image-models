...
class SparsityMap3D:
    ...
    def draw(self):
        ...
        ax.scatter(xs, ys, zs, c=sparsities, cmap=cmap, norm=Normalize(vmin=0, vmax=1), marker='o')

        ax.set_xlabel(self.labels[0])
        ax.set_ylabel(self.labels[1])
        ax.set_zlabel(self.labels[2])

        fig.colorbar(matplotlib.cm.ScalarMappable(cmap=cmap, norm=Normalize(vim=0, vmax=1)))
...
class SparsityMap2D:
    ...
    def draw(self):
        ...
        ax.scatter(xs, ys, c=sparsities, cmap=cmap, norm=Normalize(vmin=0, vmax=1), marker='o')
